...
_Generated 2026-07-23 from `_data/assumptions.json` (asof 2026-07-20). Every cross-page industry number lives here once, with all sourced variants. When a new source disagrees, add a variant to the JSON — never silently rebase a page. Rebuild: `py _wiki/_tools/build_assumptions.py`._
...
**Unreconciled sightings** (pages matching this metric, not in the ledger's `cited_in`):
- `AEIS.md` — …** deployed at Meta (~0.7% of investment/GW), content rising with the 800V/HVDC transition. Bull case $…
...
- `GEV.md` — …DC **entitlement at 2–3x today's ~$300M/GW** scope once UPS+SST are included (credibly in '27 orders,…
...
- `ASML.md` — …in-line) | | **Memory / DRAM** | Memory HBM/DDR "sold out" '26, tight thru '26 | UBS: ASML ~30% of DRAM WFE dollars (…
...
- `themes/cowos-packaging.md` — …h the Rubin Ultra 384GB HBM4E reset on [hbm-memory.md](hbm-memory.md)). **(2) TSMC CoWoS capacity 140k wspm YE26 → 235k YE28** (total incl. SPIL 25k + Amkor 16k = **17…
...
- `GEV.md` — …, SRA **56→63GW**; gas backlog+SRA 100→116GW, ≥125GW by YE. **$/kw spiked ~$700 (1Q) → $1,043 (2Q)** (investors'…
